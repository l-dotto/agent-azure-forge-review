...
Executes the Forge code review agent for architecture, quality,
maintainability, and performance analysis using Claude Opus 4.5.

Supported LLM Providers:
- Anthropic Claude (default - uses Opus 4.5 for deeper analysis)
- OpenAI GPT-4
- Azure OpenAI
- Google Gemini
"""
...
class CodeReviewRunner:
    """Runner for Code Review Agent (Forge)"""

    DEFAULT_MODELS = {
        'anthropic': 'claude-opus-4-5-20251101',  # Opus for deeper analysis
    ...
    def __init__(
        ...
        agent_path = Path(__file__).parent.parent.parent / '.claude' / 'agents' / 'pragmatic-code-review-subagent.md'
        if not agent_path.exists():
            raise FileNotFoundError(f"Code review agent not found: {agent_path}")
    ...
    def run(self, repo_path: str, output_file: Optional[str] = None, target_branch: str = 'main') -> Dict[str, Any]:
        ...
        with Progress(
            ...
            if not diff_result['diff']:
                console.print("[yellow]No changes detected, skipping code review[/yellow]")
                return {
                    "findings": [],
                    "metadata": {
                        "agent": "forge",
            ...
            task2 = progress.add_task(f"[cyan]Analyzing code (model: {self.model})...", total=None)
            system_prompt = "You are Forge, a pragmatic senior code reviewer focusing on architecture, maintainability, and substantive issues following the 'Net Positive over Perfection' framework."
            ...
            findings = parse_agent_output(markdown_output, 'code')
        ...
        console.print(f"Found {len(findings)} code quality findings:")
...
def main():
    parser = argparse.ArgumentParser(
        description='Execute Forge code review agent',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic usage (auto-detect provider from env)
  python run_code_review.py --repo . --output findings/code.json

  # Specify provider and model
  python run_code_review.py --repo . --provider anthropic --model claude-opus-4-5-20251101 --output findings/code.json

  # Compare against different branch
  python run_code_review.py --repo . --target-branch develop --output findings/code.json
...
"""
    ...
    parser.add_argument('--max-tokens', type=int, default=CodeReviewRunner.DEFAULT_MAX_TOKENS, help='Max tokens')
    parser.add_argument('--temperature', type=float, default=CodeReviewRunner.DEFAULT_TEMPERATURE, help='Temperature')
